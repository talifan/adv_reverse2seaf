...
Этот документ описывает Python-скрипт, предназначенный для конвертации данных технической архитектуры из исходного формата `cloud.ru/advanced/` в целевой формат метамодели `seaf-core/entities/ta/`. Содержимое обновлено с учётом последних улучшений (весна 2024 года), связанных с автоматическим определением размещения ресурсов и интеграцией с генератором диаграмм SEAF2DrawIO.
...
Конвертер использует файл `converter_config.yaml` для определения настроек по умолчанию. Пример содержимого файла (пути приведены в формате WSL/POSIX, чтобы их можно было без правок использовать из Ubuntu-командной строки):
...
input_dir: '/mnt/c/Users/<user>/Documents/seaf_dev/seaf-dzo-example/architecture/ta/reverse/cloud.ru/advanced'

# Output directory for the converted seaf-core entities
output_dir: '/mnt/c/Users/<user>/Documents/seaf_dev/seaf-dzo-example/architecture/ta/converted'
...
| `vpcs`                       | `seaf.ta.components.network` | VPC преобразуется в виртуальный маршрутизатор, для которого автоматически подбираются все связанные ЦОД и сегменты (по подсетям, серверам, кластерам и т.д.). |
| `subnets`                    | `seaf.ta.services.network`   | Подсеть преобразуется в сеть типа "LAN" или "WAN". Привязка к ЦОД выполняется автоматически на основе агрегированных подсказок (AZ, DC, связанные ресурсы). |
...
| `eips`                       | `seaf.ta.services.network`, `seaf.ta.services.network_links` | Elastic IP преобразуются в сети типа "WAN" и сетевые связи. Сегмент (INTERNET/INT-NET) определяется автоматически по внутреннему адресу. |
...
## 5. Типовой процесс работы

1.  Запустить `converter.py` с нужной конфигурацией (по умолчанию — `converter_config.yaml` рядом со скриптом).
2.  Проверить файлы в `architecture/ta/converted/`: убедитесь, что появились новые YAML для всех сущностей и корректно обновился `root.yaml`.
3.  При необходимости внести ручные правки: заполнить дополнительные атрибуты, добавить связи, которых нет во входных данных.
4.  Перейти в каталог `SEAF2DrawIO/` и запустить генерацию диаграмм:
    ```bash
    python3 seaf2drawio.py
    ```
    Скрипт читает YAML прямо из `../seaf-dzo-example/architecture/ta/converted/`, поэтому дополнительное копирование данных не требуется.
5.  Открыть файл `SEAF2DrawIO/result/reverse.drawio` в draw.io и визуально проверить результат.

## 6. Проверка результатов

*   **Соответствие данных:** названия, идентификаторы и описания должны совпадать с исходными объектами.
*   **Связи и зависимости:** поля `network_connection`, `availabilityzone`, `location`, `segment` генерируются автоматически. Если каких-то ссылок нет, значит входные данные не содержали достаточных подсказок — проверьте `mapping_analysis.md`.
*   **Диаграмма:** после выполнения `seaf2drawio.py` обратите внимание на финальный отчёт. Все сущности должны иметь статус `OK`; предупреждения допустимы только для элементов, отсутствующих в данных (например, внешние сегменты).
...
Все конвертеры используют общий префикс (по умолчанию `tenant`). Его можно задать через аргумент `--id-prefix` или в `converter_config.yaml`. Если не указать, он будет автоматически определён по исходным ключам.

## 10. Что нового в актуальной версии

* Добавлен вспомогательный модуль `utils/location_resolver.py`, агрегирующий подсказки о расположении (AZ, DC, VPC, Subnet) и обеспечивающий единообразные `location`, `segment`, `availabilityzone`.
* Предопределённые сетевые сегменты получают поле `zone`, благодаря чему `SEAF2DrawIO` размещает объекты на правильных страницах.
* Балансировщики (ELB) перемещены в `seaf.ta.services.compute_service`, что соответствует визуальному представлению диаграмм и избавляет от предупреждений о типах.
* VPN-соединения формируются только для тех конечных точек, которые представлены в модели: внешние DC/офисы без описания больше не создают «висящие» связи в YAML и на диаграмме.
* Файл `SEAF2DrawIO/config.yaml` теперь читает YAML непосредственно из каталога `architecture/ta/converted/`, что исключает ручные копии данных перед генерацией.
