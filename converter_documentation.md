...
| `eips`                       | `seaf.ta.services.network`, `seaf.ta.services.network_links`, `seaf.ta.services.network_segment` | Elastic IP преобразуются в сети типа "WAN", подключаются к интернет-сегментам по каждому ЦОДу (имена зависят от префикса) и создают сетевые связи к внутренним ресурсам по `int_address`. |
