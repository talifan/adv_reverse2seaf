...
**Анализ сопоставления свойств (при конвертации в `network` с маской /32):**
...
| `id` | `external_id` | Прямое сопоставление. |
| `ext_address` | `cidr` | Можно создать CIDR с маской /32, например, "73.54.34.19/32". |
| (статическое значение) | `type` | Установить в "WAN". |
| `limit.throughput` | `throughput` | Прямое сопоставление. |
| `type`, `port_id`, `int_address`, `limit` (кроме `throughput`), `tenant`, `DC` | (нет) | Эти поля не имеют прямых аналогов. Информация о NAT-связи с внутренним IP теряется. |
| (нет) | `vlan`, `segment`, `is_management`, `is_own`, `provider`, `redundancy` | Эти поля в целевой сущности нужно будет заполнять на основе дополнительной логики или вручную. |

**Ключевые моменты и рекомендации:**

1.  **Концептуальное несоответствие (IP vs. Сеть):**
    *   **Проблема:** `eips` — это, по сути, правило NAT "один к одному", а не сеть. Преобразование его в `seaf.ta.services.network` является сильным упрощением. `eips` связывает внешний IP (`ext_address`) с внутренним (`int_address`). Эта информация о связи теряется при простой конвертации в сеть.
    *   **Рекомендация 1 (Простая):** Создавать сущность `seaf.ta.services.network` с типом "WAN" и CIDR с маской /32 (например, "73.54.34.19/32"). Информацию о внутреннем IP и порте (`int_address`, `port_id`) сохранять в `description`. Это простой, но неполный подход.
    *   **Рекомендация 2 (Более корректная):** Не создавать для `eips` отдельную сущность `network`. Вместо этого, `eips` следует рассматривать как **атрибут** или **связь** для другой сущности. Например, для сервера (`ecss`), у которого есть этот `eips`, можно добавить информацию о публичном IP в его `description`. Это лучше отражает реальность, где EIP — это свойство, назначаемое ресурсу.
