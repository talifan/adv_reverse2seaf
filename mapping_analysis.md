...
2.  **Группировка публичных IP:**
    ...
    *   **Рекомендация:** Создавать отдельные сущности `seaf.ta.services.network_segment` для каждого ЦОДа (location = соответствующий `flix.dc.*`) с `title: "Public Internet"` и `zone: "INTERNET"`. WAN-сети публичных EIP должны ссылаться на полный перечень этих сегментов.
