...
| `DC` | `location` | **Изменено:** Теперь `location` формируется на основе `masters_az` и ссылается на автоматически созданный ЦОД (например, `flix.dc.ru-moscow-1a`). |
...
| `DC` | `location` | **Изменено:** Теперь `location` формируется на основе `available_az` и ссылается на автоматически созданный ЦОД (например, `flix.dc.ru-moscow-1a`). |
| `subnet_id` | `network_connection` | Необходимо найти `seaf.ta.services.network` по `subnet_id` и сформировать массив из одной ссылки. |
...
2.  **Группировка публичных IP:**
    ...
    *   **Рекомендация:** Создать единую сущность `seaf.ta.services.network_segment` с `title: "Public Internet"` и `zone: "INTERNET"`. Для каждого EIP с публичным адресом, в поле `segment` сконвертированной сущности `seaf.ta.services.network` должна быть установлена ссылка на этот интернет-сегмент. Для EIP с частными адресами эта логика не применяется.
...
| `availability_zone` (также через подсеть) | `location` | Для каждого ELB определяется набор AZ, преобразуемых в ссылки на `flix.dc.*`. |
...
3.  **Заполнение обязательных полей:**
    *   **Рекомендация:** Необходимо определить логику заполнения полей `az` и `virtualization`. `az` можно определить по `subnet_id`, а `location` формируется из ссылок на `flix.dc.*`, рассчитанных по AZ подсети. Поле `virtualization` — на основе `DC` или общих правил.
...
| `subnet_id` (через `subnets`->`vpc`) | `segment` | **Ключевая логика.** Необходимо найти `network_segment`, соответствующий `vpc`, к которому принадлежит `subnet`, и сослаться на него. |
| `availability_zone` (через подсеть) | `location` | На основании AZ подсети вычисляются ссылки на ЦОДы `flix.dc.*`. |
...
3.  **Заполнение обязательных полей:**
    *   **Рекомендация:** Необходимо определить логику заполнения полей `az` и `virtualization`. `az` можно определить по `subnet_id`, `location` формируется из `flix.dc.*` на основе AZ, а `virtualization` — на основе `DC` или общих правил.
...
| `DC` | `location` | **Изменено:** Теперь `location` формируется на основе `az` и ссылается на автоматически созданный ЦОД (например, `flix.dc.ru-moscow-1a`). |
| `subnet_id` | `network_connection` | Необходимо найти `seaf.ta.services.network` по `subnet_id` и сформировать массив из одной ссылки. |
...
1.  **Маршрутизатор как агрегатор сетевых подключений:**
    ...
    *   Свойство `segment` заполняется строкой с идентификатором созданного сетевого сегмента, а `location` формируется списком ссылок на ЦОДы `flix.dc.*`, рассчитанными по зонам доступности связанных ресурсов (subnet/ECS/CCE/RDS/DMS).
...
| `availability_zone` (через подсеть, ВМ) | `location` | Определить AZ и преобразовать в ссылки на `flix.dc.*`. |
...
3.  **Заполнение обязательных полей:**
    *   **Рекомендация:** Необходимо определить логику заполнения полей `az` и `virtualization`. `az` можно определить по `subnet_id` и связанным ВМ, сформировав `location` как список ссылок `flix.dc.*`. Поле `virtualization` задаётся согласно общим правилам.
