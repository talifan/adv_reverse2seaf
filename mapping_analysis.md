...
- **Описание сопоставления:** VPC Peering (связь между двумя VPC) отображается в сущность "Логическая связь". Связь устанавливается между виртуальными маршрутизаторами, которые представляют каждый VPC.
...
| `description` | `description` | Прямое сопоставление. |
| `id` | `external_id` | Прямое сопоставление. |
| `request_vpc` | `source` | **Ключевая логика.** `request_vpc` становится источником связи. Необходимо найти **маршрутизатор**, соответствующий этому VPC ID. |
| `accept_vpc` | `target` | **Ключевая логика.** `accept_vpc` становится приемником связи. Необходимо найти **маршрутизатор**, соответствующий этому VPC ID. |
...
1.  **Разрешение конечных точек (`request_vpc` -> `source`, `accept_vpc` -> `target`):**
    *   **Логика:** `peering` связывает два `vpc`. В целевой модели `logical_link` связывает два маршрутизатора (`seaf.ta.components.network`), которые были созданы для каждого `vpc`.
    *   **Рекомендация:** Конвертер `peerings` должен формировать ссылки на маршрутизаторы. Ключ маршрутизатора строится по предсказуемому правилу: `<prefix>.vpcs.<vpc_id>.router`. Это позволяет напрямую создавать ссылки, не выполняя поиск.
